...
# office_data_diff = np.reshape((df.loc[rolling:,:]-df_ma.loc[rolling:,:]).to_numpy(),(df.loc[rolling:,:].shape[0],180,110)) # remove avg images - no background noize kind
...
ims = []
fig = plt.figure()
for i in range(rolling, rolling+1):
    # im = plt.imshow(np.clip(abs(office_data[i, :, :]-office_data[i-1, :, :]), a_min=0, a_max=None))
    original_frame = office_data[i, :, :]
    #todo save to file
    # matplotlib.image.imsave('orignal_1.png', original_frame)

    # frames_diff = office_data_diff[i, :, :]
    # matplotlib.image.imsave('frames_diff_1.png', frames_diff)
    #todo save

    frame_transposed_flipped = np.flip(np.transpose(original_frame))
    # matplotlib.image.imsave('frame_transposed_flipped_1.png', frame_transposed_flipped)
    #todo save to file
    ...
    #todo save
    # matplotlib.image.imsave('clean_1.png', details_removed)
